...
import linksRe, { any as linksAny } from '@constants/link';
import React from 'react';
// import { validate_account_name } from 'app/utils/ChainValidation';
import proxifyImageUrl from '@network/proxify';
import * as Phishing from '@constants/Phishing';
...
function linkify(content, mutate, hashtags, usertags, images, links) {
    ...
    content = content.replace(/(^|\s)(#[-a-z\d]+)/gi, tag => {
        ...
        return space + `<a href="/label/${tagLower}">${tag}</a>`;
    });
    ...
    content = content.replace(
        ...
        (match, preceeding1, preceeding2, user) => {
            ...
            return valid
                ? `${preceedings}<a href="/user/${userLower}">@${user}</a>`
                : `${preceedings}@${user}`;
        }
    );
...
export function ExhumeHtml(html, large = true) {
    let idx = 0;
    const sections = [];
    ...
    for (let section of html.split('~~~ embed:')) {
        ...
        if (match && match.length >= 3) {
            ...
            if (type === 'youtube') {
                ...
                throw new Error('MarkdownViewer unknown embed type', type);
            }
