...
import EmbedLink from '@network/html';
import tt from 'counterpart';

export const sanitize_html_text = body => {
    // if (!body.match(/^<html>[\s\S]*<\/html>$/))
    //     throw new Error(
    //         'HTML posts must begin with <html> and end with </html>'
    //     );

    let rtags = EmbedLink(body, { mutate: false });
    ...
    if (sanitizeErrors.length > 0) throw new Error(sanitizeErrors.join('.  '));

    return body;
};

export const clean_text_from_sanitize = (
    text,
    large = false,
    ...
    if (text) text = EmbedLink(text, { noImage }).html;
...
export const isNoImageActive = noImageText => {
    return cleanText.indexOf(noImageText) !== -1;
};
