...
class LabelInterestBasedRecommendEngine(Engine):
    ...
    def run(self, interests, target_label):
        self.products = interests.toSortArrayWithLabel()
        try:
            self.target_product = self.products.get(target_label.id)
            if self.target_product == None:
                return { "labels": [{ "id": k } for k in self.products.keys()] }
            data = self.exec_jaccard()
            t_data = sorted(data.items(), key=lambda x: -x[1])
            return { "labels": [{ "id": k } for k, v in t_data if target_label.id != k] }
        except:
            return { "labels": [{ "id": k } for k in self.products.keys()] }
