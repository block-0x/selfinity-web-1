...
let started;
let serverReload;
const KOA_PATH = path.join(__dirname, '../../src/backend/index');
...
const startServer = () => {
    ...
    const env = {...process.env, NODE_ENV: 'staging', BABEL_ENV: 'server'};
    ...
    server.once('message', (message) => {
        if (message.match(/^online$/)) {
            ...
            if (!started) {
                ...
                const server_path = path.join(__dirname, '../../src/backend');
